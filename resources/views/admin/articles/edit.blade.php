...
<div class="container pt-4">
    ...
    <!-- Edit News Form -->
    <form action="{{ route('admin.articles.update', $article) }}" method="POST" enctype="multipart/form-data" class="mb-4" onsubmit="return document.activeElement.id !== 'article-tags-input';">
        @csrf
        @method('PUT')
        <div class="row">
            <!-- Category -->
                ...
                <select name="state" class="form-control" id="state" required>
                    ...
                    @foreach ($emirates as $emirate)
                        <option value="{{ $emirate->name }}" 
                            {{ old('state', $article->state) == $emirate->name ? 'selected' : '' }}>
                            {{ $emirate->name }}
                        </option>
                    @endforeach
                </select>
                ...
                <select name="country" class="form-control" id="country" required>
                    ...
                    @foreach ($countries as $country)
                        <option value="{{ $country->name }}" 
                            {{ old('country', $article->country) == $country->name ? 'selected' : '' }}>
                            {{ $country->name }}
                        </option>
                    @endforeach
                </select>
        ...
        <div class="row">
            ...
            <div class="col-md-6 mb-3">
                ...
                <input type="date" 
                       ...
                       class="form-control" 
                       id="date" 
                       value="{{ old('date', optional($article)->date ? optional($article)->date->format('Y-m-d') : '') }}" 
                       required>
            </div>
        ...
        <!-- Tags Section -->
        <x-tag-input :existing-tags="$existingTags" :tags="$article->tags" name="tags" label="Tags" id="article-tags" />
