    ...
    <main>
            ...
            <div class="container">
                <div class="breadcrumb__content">
                    ...
                    <ul class="bread-crumb clearfix ul_li_center blog-flex-column">
                        ...
                        <li class="breadcrumb-item blog-title-description">{{ $article->title }}</li>
                    </ul>
                </div>
            </div>
        ...
        <section class="blog pb-130 blog-content">
            <div class="blog-back-button pt-55 pb-50 container">
                <a href="{{ url('/articles') }}" class="blog-arrow-container">
                    <img src="{{ asset('/assets/img/blog/arrow-back.png') }}" alt="Arrow Icon">
                </a>
                <h5>
                    Home / ARTICLE GALLERY / <span>{{ \Illuminate\Support\Str::limit($article->title, 16) }}</span>
                </h5>
            </div>
            <div class="container">
                            ...
                            <article class="blog__item blog-details-desciption mt-30">
                                <a class="thumb blog-desktop-thumb">
                                    <img src="{{ asset('storage/' . $article->thumbnail) }}" alt="Article Image"
                                        class="desktop">
                                    <img src="{{ asset('storage/' . $article->mobile_thumbnail) }}" alt="Article Image"
                                        class="mobile">
                                ...
                                <div class="blog__inner blog-details-inner">
                                    <ul class="blog__meta ul_li mb-30 blog-meta-data">
                                        <li><a href="https://www.google.com/maps?q={{ urlencode($article->state . ' ' . $article->country) }}"
                                                target="_blank"><i class="far fa-map-marker-alt"></i>{{ $article->state }},
                                                {{ $article->country }}</a></li>
                                        <li><i
                                                class="far fa-clock"></i>{{ \Carbon\Carbon::parse($article->date)->format('M d, Y') }}
                                        </li>
                                        <li><a><i class="far fa-comment"></i>({{ $article->comments_count }}) Comments</a>
                                        </li>
                                    </ul>
                                    <h2 class="title border_effect"><a href="blog-single.html">{{ $article->title }}</a>
                                    </h2>
                                    <p>
                                        {{ $article->content }}
                                    </p>
                                    <div class="blog-gallery-container mt-20">
                                        @foreach ($article->galleries as $articleGalleries)
                                            <img src="{{ asset('storage/' . $articleGalleries->image_path) }}"
                                                alt="Article Gallery">
                                        @endforeach
                                    </div>
                                        ...
                                        <div class="d-flex align-items-center">
                                            ...
                                            <div class="tagcloud">
                                                @forelse ($article->tags as $tag)
                                                    <a
                                                        href="{{ url('articles') . '?tags=' . urlencode($tag->id) }}">{{ $tag->name }}</a>
                                                @empty
